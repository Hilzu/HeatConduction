...
double multithread_heatconduct(Array* arr, unsigned int max_iters)
{
	...
	sem_init(&poo, 0, 0);
	for (int i = 0; i < NUM_THREADS; ++i) {
		if ((rc = pthread_create(&thr[i], NULL, my_thread_function, &thr_data[i]))) {
	...
	for (unsigned int f = 0; f < max_iters; ++f)
	{
		...
		swap_ptrs((void**) &arr, (void**) &temp_arr);
		//printf("Next round, reading from %p\n", (void*)arr);
		//printf("Next round, writing to   %p\n", (void*)temp_arr);
		for (int i = 0; i < NUM_THREADS; ++i) {
			sem_post(&food);
		}
		double new_mean = calculate_sum(temp_arr);

		//printf("Reading mean from %p\n", (void*)arr);
		if (conf.verbose) {
			printf("Iter: %d Mean: %.15f\n", f + 1, new_mean);
			if (conf.verbose > 1) {
				print_arr(arr);
		...
		if (fabs(new_mean - prev_mean) < 0.0000000000001 && f > 3) {
			printf("Found balance after %d iterations.\n", f+1);
			running = 0;
			for (int i = 0; i < NUM_THREADS; ++i) {
				sem_post(&food);
